...
const FlowingMenu: React.FC<FlowingMenuProps> = ({ items = [], paragraph, categories=[]}) => {
  return (
            ...
            <ul className="flex space-x-4 mb-4">
                {categories.map((text, i) => (
                     <li key={i} className="text-lg">
                        <span className={tiaraFont.className}>{text.slice(0,1)}</span>
                        <span className={tiaraFont.className}>{text.slice(1)}</span>
                    </li>
                ))}
            </ul>
        )}
...
const MenuItem: React.FC<MenuItemProps & {paragraph?: string}> = ({ link, text, image, paragraph }) => {
