...
const Checkbox: React.FC<CheckboxProps> = ({ className, value, checked, onChange }) => {
    return (
        <Input type="checkbox" className={className} value={value} checked={checked} onChange={onChange} />
    );
};
                            ...
                            {technical.map((event) => (
                                <div key={event.key} className="flex justify-between items-center p-4 mb-2">
                                    ...
                                    <Checkbox
                                        ...
                                        value={event.key}
                                        checked={technicalCheckedItems.some((item) => item.key === event.key)}
                            ...
                            {cultural.map((event) => (
                                <div key={event.key} className="flex justify-between items-center p-4 mb-2">
                                    ...
                                    <Checkbox
                                        className="w-6 h-6 mr-2"
                                        value={event.key}
                                        checked={culturalCheckedItems.some((item) => item.key === event.key)}
                            ...
                            {mega.map((event) => (
                                <div key={event.key} className="flex justify-between items-center p-4 mb-2">
                                    ...
                                    <Checkbox
                                        className="w-6 h-6 mr-2"
                                        value={event.key}
                                        checked={megaCheckedItems.some((item) => item.key === event.key)}
