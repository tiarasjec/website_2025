...
import { DataTableInputDate } from "@/components/datatable/data-table-input-date";
...
export function DataTableHeader<T>({ header }: { header: Header<T, unknown> }) {
    ...
    if (isSelecting) {
        return (
            <TableHead className={"text-xs"} colSpan={header.colSpan} ref={setNodeRef}>
    ...
    return (
        <TableHead className={"p-0 pl-1"} colSpan={header.colSpan} ref={setNodeRef} style={combinedStyles}>
            <ContextMenu>
                <ContextMenuTrigger>
                    <div className={"flex flex-row items-center space-x-1"}>
                            ...
                            <TooltipTrigger>
                                {isPlaceholder ? null : (
                                    <Button
                                        ...
                                        variant="ghost"
                                        size="sm"
                                        className="h-7 data-[state=open]:bg-accent"
                                    >
                                        {flexRender(column.columnDef.header, header.getContext())}
                                        {column.getIsSorted() === "desc" ? (
                        ...
                        <Tooltip>
                            <TooltipTrigger>
                                {column.getIsPinned() ? (
                                    <PinOffIcon className="h-4 w-4" onClick={() => column.pin(false)} />
                                ) : (
                                    <PinIcon className="h-4 w-4" onClick={() => column.pin("left")} />
                                )}
                            </TooltipTrigger>
                            <TooltipContent>
